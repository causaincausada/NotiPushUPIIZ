<?php
    ...
    function validar_Password()
    {
        ...
        existe_empleado($numempleado);
        $respuesta['encontrado'] = true;

        $link=connect();
        $consulta="SELECT * FROM `empleado` WHERE `numempleado` LIKE "."'". $numempleado ."'";
    ...
    function existe_empleado($numempleado)
    {
        $link=connect();
        $consulta="SELECT COUNT(*) FROM `empleado` WHERE `numempleado` LIKE '". $numempleado ."'";
        $r = mysqli_query($link, $consulta) or error_Consulta();
        mysqli_close($link);
